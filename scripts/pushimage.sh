...
h2 "Login to Docker registry"
DOCKER_LOGIN="docker login -u $USERNAME -p $PASSWORD $REGISTRY"
info "$DOCKER_LOGIN"
DOCKER_LOGIN_OUTPUT=$($DOCKER_LOGIN)

if [ $? -ne 0 ];then
  warn $DOCKER_LOGIN_OUTPUT
  error "Login to registry server $REGISTRY failed";
else
  success "Login to registry server $REGISTRY succeeded";
fi
...
h2 "Logout from Docker registry"
...
if [ $? -ne 0 ];then
  ...
  error "Logout from registry server $REGISTRY failed";
else
  success "Logout from registry server $REGISTRY succeeded";
fi
